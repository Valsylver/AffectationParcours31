...
function updatePaSelect(value, id) {
	if (value != "------------------------- Pas de choix -------------------------") {
		var idChanged = parseInt(id[id.length - 1]);
		
		// Recherche du groupe de l'option à supprimer 
		var realGroup;
		var currentSelect = document.getElementById("selectPa" + currentId);
		var groups = currentSelect.getElementsByTagName("optgroup");
		var ind = 0;
		for (var i=0; i<groups.length; i++) {
			...
			for (var j=0; j<options.length; j++){
				...
				if (option.text == value){
					realGroup = group.label;
				}
			}
		}
		
		// Suppression de l'option des autres select
		for (i = 1; i < 6; i++) {
		...
		// Recuperation du select qui a changé et d'un autre select
		...
		// Recuperation des options du select qui a changé et des options d'un autre
		...
		// Recherche si une option est manquante dans les autres select
			...
			for (i = 1; i < 6; i++) {
				if (i != parseInt(id[id.length - 1])) {
					...
					for (var j=0; j<select_groups.length; j++){
						...
						if (group.label == realGroup){
							var options = group.getElementsByTagName("option");
							if (optionManquante < options[1].text) {
								where = currentWhere;
							} else {
								if (optionManquante > options[options.length - 1].text) {
									if (j==select_groups.length-1){
										...
										where = currentWhere+group.options.length;
									}
								} else {
							...
							currentWhere += group.options.length;
						}
					}
