		...
		<div class="row">
			<div class="span2">
				<ul class="nav nav-list">
					...
					<c:forEach var="spec" items="${allPa}" varStatus="status">
					...
					<c:forEach var="spec" items="${allFm}" varStatus="status">
			...
			<div class="span7">
				<ul class="nav nav-pills">
					<c:forEach var="i" begin="1" end="5" step="1">
						<c:choose>
							<c:when test="${i == order}">
								<li class="active">
									<a href="/admin/parcours/synthese/choix${i}">Choix ${i}</a>
								</li>
							</c:when>
							<c:otherwise>
								<li>
				...
				<c:forEach var="specialization" items="${allPa}" varStatus="status">
					<span class="lead">
						<a href="/admin/config/modif/parcours/${specialization.abbreviation}" >
					...
					<c:choose>
						...
						<c:otherwise>
							<p>Aucun elève n'a pour l'instant choisi ce parcours d'approfondissement en 
								<c:choose>
									<c:when test="${order == 1}">
										1er choix
									</c:when>
