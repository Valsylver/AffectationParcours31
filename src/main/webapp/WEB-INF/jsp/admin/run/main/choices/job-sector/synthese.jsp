			...
			<div class="span8">
				<ul class="nav nav-pills">
					<c:forEach var="i" begin="1" end="5" step="1">
						<c:choose>
							<c:when test="${i == order}">
								<li class="active"><a href="/admin/run/main/choices/job-sector/synthese/choice${order}">Choix ${i}</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="/admin/run/main/choices/job-sector/synthese/choice${i}">Choix ${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</ul>
				...
				<c:forEach var="specialization" items="${allJs}" varStatus="status">
					<span class="lead"> <a href="/admin/common/edit/js/${specialization.abbreviation}"> ${specialization.name} (${specialization.abbreviation}) </a>
					</span>
					<br />
					<br />
					<c:choose>
