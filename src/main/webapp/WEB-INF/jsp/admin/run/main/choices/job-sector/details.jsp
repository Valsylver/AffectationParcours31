					...
					<c:forEach var="spec" items="${allJs}" varStatus="status">
						<c:choose>
							<c:when test="${spec.abbreviation == abbreviation}">
								<li class="active"><a href="${pageContext.request.contextPath}/admin/run/main/choices/job-sector/details/${abbreviation}/choice1">${spec.abbreviation}</a></li>
							</c:when>
							<c:otherwise>
			...
			<div class="span7">
				<ul class="nav nav-pills">
					<c:forEach var="i" begin="1" end="5" step="1">
						<c:choose>
							<c:when test="${i == order}">
								<li class="active"><a href="${pageContext.request.contextPath}/admin/run/main/choices/job-sector/details/${abbreviation}/choice${i}">Choix ${i}</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="${pageContext.request.contextPath}/admin/run/main/choices/job-sector/details/${abbreviation}/choice${i}">Choix ${i}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
				</ul>
