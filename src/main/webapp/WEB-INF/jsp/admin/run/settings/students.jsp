					...
					<div class="row">	
						<div class="span3">
							...
							<c:forEach var="student" items="${studentsConcerned}"
								varStatus="status">
								<li>
								<a id="promo${status.index}" title="${student.login}"
									href="javascript:void(0);" onclick="removeStudent(this.id);"><img
									src="/img/minus-new.png"></img></a> ${student.name}
								</li>
							</c:forEach>
							</ul>
						</div>
							...
							<ul id="exclusion" class="unstyled">
								<c:forEach var="student" items="${studentsToExclude}"
									...
									<li><a id="exclusion${status.index}"
										...
										onclick="addStudent(this.id);"><img
											src="/img/plus-new.png"></img></a> ${student.name}</li>
								</c:forEach>
							</ul>
						</div>
						<div class="span4">
