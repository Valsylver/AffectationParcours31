...
<body>
	<div class="container">
		...
		<div class="row">
			<div class="span8 offset2">	            
	            <form:form action="/eleve/processForm" method="POST" commandName="fullChoice" class="well form-horizontal" enctype="multipart/form-data">
						<legend><h3>Parcours d'approfondissement</h3></legend>      						
      						<c:choose>
      							<c:when test="${choiceIc == null}">
		      						<% for (int i=1; i<6; i++){String strChPa = "improvementCourseChoice.choice" + i; String strAttrPa = "Choix " + i; 
						...
						<legend><h3>Filières métier</h3></legend>      						
      						<c:choose>
      							<c:when test="${choiceJs == null}">
		      						<% for (int i=1; i<6; i++){String strChFm = "jobSectorChoice.choice" + i; String strAttrFm = "Choix " + i; 
						...
						<c:choose>
							<c:when test="${!hasFilledResume}">
								<span id="resume2">
									...
									<div class="input-append">
										<input id="resumeInput" type="text" style="min-width:300px">
										<a class="btn" onclick="$('input[id=resume]').click();">Parcourir...</a>
									</div>
									<br />
									<br />
								</span>
							...
							<c:otherwise>
								<span id="resume2">
									<label><h4>CV</h4></label>
									Vous avez déjà déposé votre CV, voulez vous supprimer ce fichier pour en ajouter un nouveau ? 
									<a class="btn" onclick="changeResume();">Oui</a>
						...
						<c:choose>
							<c:when test="${!hasFilledLetterIc}">
								<span id="letterIc2">
									<label for="letterIc"><h4>Lettre de motivation parcours d'approfondissement</h4></label>
									<input id="letterIc" name="letterIc" type="file" style="display:none">
									<div class="input-append">
										<input id="letterIcInput" type="text" style="min-width:300px">
										<a class="btn" onclick="$('input[id=letterIc]').click();">Parcourir...</a>
									</div>
									<br />
									<br />
								</span>
							...
							<c:otherwise>
								<span id="letterIc2">
									<label><h4>Lettre de motivation parcours d'approfondissement</h4></label>
									Vous avez déjà déposé votre lettre de motivation pour le parcours d'approfondissement,
									voulez vous supprimer ce fichier pour en ajouter un nouveau ? 
									<a class="btn" onclick="changeLetterIc();">Oui</a>
									<div style="display:none">
										<input id="letterIc" name="letterIc" type="file" style="display:none">
										<div class="input-append">
											<input id="letterIcInput" type="text" style="min-width:300px">
						...
						<c:choose>
							<c:when test="${!hasFilledLetterJs}">
								<span id="letterJs2">
									...
									<div class="input-append">
										<input id="letterJsInput" type="text" style="min-width:300px">
										<a class="btn" onclick="$('input[id=letterJs]').click();">Parcourir...</a>
									</div>
									<br />
									<br />
								</span>
							...
							<c:otherwise>
								<span id="letterJs2">
									<label><h4>Lettre de motivation filière métier</h4></label>
									Vous avez déjà déposé votre lettre de motivation pour la filière métier,
									voulez vous supprimer ce fichier pour en ajouter un nouveau ? 
									<a class="btn" onclick="changeLetterJs();">Oui</a>
									<div style="display:none">
										<input id="letterJs" name="letterJs" type="file" style="display:none">
										<div class="input-append">
											<input id="letterJsInput" type="text" style="min-width:300px">
        				...
        				<button class="btn btn-primary pull-right" name="commit" type="submit"><i class="icon-white icon-ok"></i> Valider</button>
        				<br />
    ...
    <script src="/js/dynamic_select.js" type="text/javascript"></script>
