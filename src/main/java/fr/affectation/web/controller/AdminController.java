...
public class AdminController {
	...
	public String redirect(Model mode) {
		return configurationService.isRunning() ? "redirect:/admin/run/main/statistics/choice1"
	...
	// @RequestMapping(value = "/common/update-agap", method =
	// RequestMethod.GET)
	// public @ResponseBody
	// void updateAgap() {
	...
	public @ResponseBody
	void inverseValidation(@RequestParam("type") String type,
			@RequestParam("login") String login,
			@RequestParam("currentValidation") String currentValidation) {
		if (type.equals("ic")) {
			validationService.updateIcValidation(login,
					!currentValidation.equals("true"));
		} else if (type.equals("js")) {
			validationService.updateJsValidation(login,
					!currentValidation.equals("true"));
		}
	}
	...
	public String excludeStudent(Model model) {
		if (!configurationService.isRunning()) {
			...
			model.addAttribute("studentsConcerned",
					studentService.findCurrentPromotionStudentsConcerned());
			model.addAttribute("studentsToExclude",
					studentService.findAllStudentsToExclude());
			model.addAttribute("studentsCesure",
					studentService.findCesureStudentsConcerned());
			model.addAttribute("mail1Activated",
					configurationService.isFirstMailActivated());
			model.addAttribute("mail2Activated",
					configurationService.isSecondMailActivated());
		...
		if (!configurationService.isRunning()) {
			if (exclusion.isEmpty()) {
				...
				condition = exclusion.getContentType().equals(
						...
						|| exclusion.getContentType().equals(
								"application/msexcel");
				if (!condition) {
					redirectAttributes.addFlashAttribute("alertMessage",
					...
					if (studentService
							.populateStudentToExcludeFromFile(exclusion)) {
	...
	public String configurationIndex(Model model) {
		if (!configurationService.isRunning()) {
			...
			model.addAttribute("paAvailable",
					studentService.findIcAvailableAsListWithSuperIc());
			model.addAttribute("fmAvailable",
					specializationService.findJobSectors());
			Date date = new Date();
			...
			if (result.hasErrors()) {
				...
				if (!areDatesSuccessive) {
					...
					model.addAttribute("alertMessage",
							"Les dates doivent êtres successives.");
					return "admin/config/index";
				} else {
					if (!((specializationService.findImprovementCourses()
							.size() >= 5) && (specializationService
							.findJobSectors().size() >= 5))) {
						model.addAttribute("paAvailable", studentService
								.findIcAvailableAsListWithSuperIc());
						model.addAttribute("fmAvailable",
								specializationService.findJobSectors());
						model.addAttribute(
								"alertMessage",
						...
						if (!configurationService.initialize(request
	...
	public String saveImprovementCourse(
		...
		if (result.hasErrors()) {
			model.addAttribute("state",
					configurationService.isRunning() ? "run" : "config");
			return "admin/common/edit-specialization";
		}
		...
		if (!run) {
			redirectAttributes.addFlashAttribute(
					"successIc",
					"Le parcours d'approfondissement <b>"
							+ specialization.getName()
							+ "</b> a bien été modifié ou crée.");
		}
		return run ? "redirect:/admin/run/settings/specializations"
				: "redirect:/admin/config";
	}
	...
	public String saveJobSector(
		...
		boolean run = configurationService.isRunning();
		if (!run) {
			redirectAttributes.addFlashAttribute("successJs",
					"La filière métier <b>" + specialization.getName()
							+ "</b> a bien été modifié ou crée.");
		}
		return run ? "redirect:/admin/run/settings/specializations"
				: "redirect:/admin/config";
	...
	public String displayStudent(@PathVariable String login, Model model,
		...
		if (configurationService.isRunning()) {
			model.addAttribute(
					"student",
					studentService.retrieveStudentByLogin(login, request
							.getSession().getServletContext().getRealPath("/")));
			model.addAttribute("allIc",
					specializationService.findImprovementCourses());
			model.addAttribute("allJs", specializationService.findJobSectors());
			boolean validationAvailable = !configurationService
					.isSubmissionAvailable();
			model.addAttribute("validationAvailable", validationAvailable);
			if (validationAvailable) {
				model.addAttribute("isValidatedIc", validationService
						.isValidatedIc(login) ? "true" : "false");
				model.addAttribute("isValidatedJs", validationService
						.isValidatedJs(login) ? "true" : "false");
			}
	...
	public String add(Model model, @PathVariable String login,
			HttpServletRequest request) {
		if (configurationService.isRunning()) {
			String path = request.getSession().getServletContext()
					.getRealPath("/");
			Choice choiceIc = choiceService
					.findImprovementCourseChoiceByLogin(login);
	...
	public String processForm(
		...
		if (configurationService.isRunning()) {
			...
			ImprovementCourseChoice improvementCourseChoice = fullChoice
					.getImprovementCourseChoice();
			...
			improvementCourseChoice.setChoice1(specializationService
					.getAbbreviationFromStringForForm(improvementCourseChoice
							.getChoice1()));
			improvementCourseChoice.setChoice2(specializationService
					.getAbbreviationFromStringForForm(improvementCourseChoice
							.getChoice2()));
			improvementCourseChoice.setChoice3(specializationService
					.getAbbreviationFromStringForForm(improvementCourseChoice
							.getChoice3()));
			improvementCourseChoice.setChoice4(specializationService
					.getAbbreviationFromStringForForm(improvementCourseChoice
							.getChoice4()));
			improvementCourseChoice.setChoice5(specializationService
					.getAbbreviationFromStringForForm(improvementCourseChoice
							.getChoice5()));

			jobSectorChoice.setChoice1(specializationService
					.getAbbreviationFromStringForForm(jobSectorChoice
							.getChoice1()));
			jobSectorChoice.setChoice2(specializationService
					.getAbbreviationFromStringForForm(jobSectorChoice
							.getChoice2()));
			jobSectorChoice.setChoice3(specializationService
					.getAbbreviationFromStringForForm(jobSectorChoice
							.getChoice3()));
			jobSectorChoice.setChoice4(specializationService
					.getAbbreviationFromStringForForm(jobSectorChoice
							.getChoice4()));
			jobSectorChoice.setChoice5(specializationService
					.getAbbreviationFromStringForForm(jobSectorChoice
							.getChoice5()));
			...
			for (String fileName : fileListName) {
				...
				if (!file.isEmpty()) {
					if (documentService.validatePdf(file)) {
						boolean cond = true;
						if (indexFile == 0) {
							cond = documentService.saveResume(path, login,
									resume);
						}
						if (indexFile == 1) {
							cond = documentService.saveLetterIc(path, login,
									letterIc);
						}
						if (indexFile == 2) {
							cond = documentService.saveLetterJs(path, login,
									letterJs);
						}
						if (!cond) {
							filesOk = false;
							redirectAttributes
									.addFlashAttribute(fileName + "Error",
											"Une erreur est survenue lors de la lecture du fichier.");
						}
					} else {
						redirectAttributes.addFlashAttribute(
			...
			if (!filesOk) {
				return "redirect:/admin/run/main/student/edit-student-form/"
						+ login;
			}

			redirectAttributes.addFlashAttribute("change",
	...
	public @ResponseBody
	String removeDocument(@RequestParam String type,
	...
	public String deleteAdmins(Model model, @PathVariable String login,
			RedirectAttributes redirectAttributes) {
		if (configurationService.isRunning()) {
			...
			if (numberOfAdmins == 1) {
				...
				redirectAttributes.addFlashAttribute("successMessage",
						"L'administrateur dont le login était " + login
								+ " a bien été supprimé.");
			}
			return "redirect:/admin/run/settings/admins";
		} else {
	...
	public String addNewAdmin(Model model, @ModelAttribute Admin admin,
			RedirectAttributes redirectAttributes) {
		if (configurationService.isRunning()) {
			if (admin.getLogin().equals("") || admin.getLogin() == null) {
				redirectAttributes
						.addFlashAttribute("alertMessage",
								"Impossible de sauvegarder un administrateur dont le login est vide.");
			} else {
				adminService.save(admin.getLogin());
				redirectAttributes.addFlashAttribute(
						"successMessage",
						"Un nouvel administrateur dont le login est "
								+ admin.getLogin() + " a bien été ajouté.");
			}
			return "redirect:/admin/run/settings/admins";
		} else {
			return "redirect:/admin";
	...
	public String manageProcess(Model model) {
		if (configurationService.isRunning()) {
			...
			model.addAttribute("modifyFirstEmail",
					calendar.before(calendarFirstEmail));
			model.addAttribute("modifySecondEmail",
					calendar.before(calendarSecondEmail));
			model.addAttribute("modifyEndSubmission",
					calendar.before(calendarEndSubmission));
			model.addAttribute("modifyEndValidation",
					calendar.before(calendarEndValidation));
			DateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm");
			model.addAttribute("mail1Activated",
					configurationService.isFirstMailActivated());
			model.addAttribute("mail2Activated",
			...
			model.addAttribute("firstEmail",
					dateFormat.format(whenToModify.getFirstEmail()));
			model.addAttribute("secondEmail",
					dateFormat.format(whenToModify.getSecondEmail()));
			model.addAttribute("endSubmission",
					dateFormat.format(whenToModify.getEndSubmission()));
			model.addAttribute("endValidation",
					dateFormat.format(whenToModify.getEndValidation()));
			model.addAttribute("when", new When());
	...
	public String editMails(Model model, HttpServletRequest request,
		...
		if (configurationService.isRunning()) {
			...
			model.addAttribute("mail2Activated",
					configurationService.isSecondMailActivated());
			model.addAttribute("mail", number == 1 ? mailService.getFirstMail()
					: mailService.getSecondMail());
			model.addAttribute("number", number);
			model.addAttribute("activated",
					number == 1 ? configurationService.isFirstMailActivated()
							: configurationService.isSecondMailActivated());
	...
	public String inverseMailActivation(Model model,
			HttpServletRequest request, @PathVariable int number) {
		if (configurationService.isRunning()
				&& ((number == 1) || (number == 2))) {
			if (number == 1) {
				configurationService
						.setFirstMailActivated(!configurationService
								.isFirstMailActivated());
			} else if (number == 2) {
				configurationService
						.setSecondMailActivated(!configurationService
								.isSecondMailActivated());
			}
	...
	public String processSpontaneousMail(
			@ModelAttribute @Valid SimpleMail simpleMail, BindingResult result,
	...
	public String saveFirstMail(@ModelAttribute @Valid Mail mail,
			BindingResult result, Model model,
			RedirectAttributes redirectAttributes) {
		if (configurationService.isRunning()) {
			if (result.hasErrors()) {
	...
	public String editProcess(@ModelAttribute When when, BindingResult result,
			Model model) {
		if (configurationService.isRunning()) {
			...
			if (when.getNumber() == 2) {
				if (when.getEndSubmission() == null) {
					FieldError fieldError = new FieldError("when",
							"endSubmission", "");
					result.addError(fieldError);
				}
				if (when.getEndValidation() == null) {
			...
			if (result.hasErrors()) {
				...
				if (!areDatesSuccessive) {
					model.addAttribute(
							"alertMessage",
							"Impossible de sauvegarder cette configuration, les dates doivent êtres successives.");
					When whenToModify = configurationService.getWhen();
					Calendar calendar = Calendar.getInstance();
					calendar.add(Calendar.HOUR, 1);
					Calendar calendarFirstEmail = Calendar.getInstance();
					...
					calendarSecondEmail.setTime(whenToModify.getSecondEmail());
					Calendar calendarEndSubmission = Calendar.getInstance();
					calendarEndSubmission.setTime(whenToModify
							.getEndSubmission());
					Calendar calendarEndValidation = Calendar.getInstance();
					calendarEndValidation.setTime(whenToModify
							.getEndValidation());
					model.addAttribute("modifyFirstEmail",
							calendar.before(calendarFirstEmail));
					model.addAttribute("modifySecondEmail",
	...
	public String stopProcess(HttpServletRequest request) {
		if (configurationService.isRunning()) {
			try {
				...
				documentService.deleteAll(request.getSession()
	...
	public String allResultsIc(@PathVariable int order, Model model) {
		if (configurationService.isRunning()) {
			...
			model.addAttribute("running",
					!configurationService.isValidationForAdminAvailable());
			model.addAttribute(
					"allStudents",
					configurationService.isValidationForAdminAvailable() ? studentService
							.findSimpleStudentsWithValidationForAllIcByOrder(order)
							: studentService
	...
	public String allResultsJs(@PathVariable int order, Model model) {
		if (configurationService.isRunning()) {
			...
			model.addAttribute(
					"allStudents",
					configurationService.isValidationForAdminAvailable() ? studentService
							.findSimpleStudentsWithValidationForAllJsByOrder(order)
							: studentService
	...
	public String resultsIcDetails(@PathVariable String abbreviation,
		...
		if (configurationService.isRunning()) {
			...
			model.addAttribute(
					"allStudents",
					configurationService.isValidationForAdminAvailable() ? studentService
							.findSimpleStudentsWithValidationByOrderChoiceAndSpecialization(
									order, improvementCourse)
							: studentService
									.findSimpleStudentsByOrderChoiceAndSpecialization(
											order, improvementCourse));
			model.addAttribute("running",
					!configurationService.isValidationForAdminAvailable());
	...
	public String pieChartsForChoice(@PathVariable int choice, Model model) {
		if (configurationService.isRunning()) {
			...
			model.addAttribute("simpleImprovementCourses",
					statisticsService.findSimpleIcStats(choice));
			model.addAttribute("simpleJobSectors",
					statisticsService.findSimpleJsStats(choice));
			model.addAttribute("allIc",
					specializationService.findImprovementCourses());
			model.addAttribute("allJs", specializationService.findJobSectors());
	...
	public String pieChartsForOtherChoice(@PathVariable int choice,
			@PathVariable String abbreviation, @PathVariable int type,
			Model model) {
		if (configurationService.isRunning()) {
			...
			model.addAttribute("specializations", studentService
					.findChoiceRepartitionKnowingOne(1, choice, specialization));
			model.addAttribute("allIc",
					specializationService.findImprovementCourses());
			model.addAttribute("allJs", specializationService.findJobSectors());
	...
	public String inverseRepartitionIc(@PathVariable String abbreviation,
			Model model) {
		if (configurationService.isRunning()) {
			Specialization specialization = specializationService
					.getImprovementCourseByAbbreviation(abbreviation);
			model.addAttribute("specialization", specialization);
			model.addAttribute("inverseSpecializations",
					studentService.findInverseRepartition(specialization));
			model.addAttribute("allIc",
					specializationService.findImprovementCourses());
			model.addAttribute("allJs", specializationService.findJobSectors());
	...
	public String inverseRepartitionJs(@PathVariable String abbreviation,
			Model model) {
		if (configurationService.isRunning()) {
			Specialization specialization = specializationService
					.getJobSectorByAbbreviation(abbreviation);
	...
	public String pieChartsForForms(HttpServletRequest request, Model model) {
		if (configurationService.isRunning()) {
			String path = request.getSession().getServletContext()
					.getRealPath("/");
			Map<String, Integer> numbersForCategories = studentService
					.findSizeOfCategories(path);
			model.addAttribute("nbreAll", numbersForCategories.get("total"));
			model.addAttribute("nbrePartial",
					numbersForCategories.get("partial"));
	...
	public String detailsForForms(HttpServletRequest request,
			@PathVariable String category, Model model) {
		if (configurationService.isRunning()) {
			String path = request.getSession().getServletContext()
					.getRealPath("/");
			List<SimpleStudent> results = studentService
					.findStudentsForCategorySynthese(category, path);
			Map<String, Integer> numbersForCategories = studentService
					.getSizeOfCategories(path);
			model.addAttribute("nbreAll", numbersForCategories.get("total"));
			model.addAttribute("nbrePartial",
					numbersForCategories.get("partial"));
			model.addAttribute("nbreNo", numbersForCategories.get("empty"));
